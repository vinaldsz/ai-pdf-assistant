...
def _r(chunk_id: str, *, doc_id: str = "doc1", page: int = 1, score: float = 0.9) -> RetrievalResult:
...
def test_rrf_chunk_in_both_lists_ranks_higher_than_exclusive_chunks():
    ...
    exclusive_positions = [i for i, r in enumerate(result) if r.chunk_id in (exclusive_dense | exclusive_sparse)]
    assert max(shared_positions) < max(exclusive_positions) or min(shared_positions) < min(exclusive_positions)
...
async def test_below_threshold_returns_empty_list():
    ...
    with patch("app.rag.store.dense_search", new_callable=AsyncMock, return_value=[low_score_row]), \
...
async def test_empty_dense_results_returns_empty_list():
    with patch("app.rag.store.dense_search", new_callable=AsyncMock, return_value=[]), \
         patch("app.rag.store.sparse_search", new_callable=AsyncMock, return_value=[]), \
...
async def test_above_threshold_returns_results():
    good_row = {"id": "uuid-1", "doc_id": "doc-1", "page": 1, "text": "relevant text", "score": 0.85}

    with patch("app.rag.store.dense_search", new_callable=AsyncMock, return_value=[good_row]), \
         patch("app.rag.store.sparse_search", new_callable=AsyncMock, return_value=[]), \
         patch("app.rag.embedder.encode_batch", return_value=[[0.5] * 384]):
        result = await retrieve("any query")
