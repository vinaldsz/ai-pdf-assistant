...
async def client():
    ...
    with (
        patch("app.rag.embedder.warmup", new_callable=AsyncMock),
...
async def test_ready_returns_503_when_db_unreachable(client: AsyncClient):
    with (
        patch("app.rag.store.get_pool", side_effect=Exception("connection refused")),
...
async def test_ready_returns_503_when_embedder_not_warm(client: AsyncClient):
    with (
        patch("app.rag.embedder._warmed", new=False),
        patch("app.rag.reranker._warmed", new=True),
...
async def test_query_empty_retrieval_does_not_call_llm(client: AsyncClient):
    with (
        patch("app.rag.retriever.retrieve", new_callable=AsyncMock, return_value=[]),
...
async def test_query_calls_reranker_before_generator(client: AsyncClient):
    ...
    with (
        patch("app.rag.retriever.retrieve", new_callable=AsyncMock, return_value=[chunk]),
        patch(
            "app.rag.reranker.rerank", new_callable=AsyncMock, return_value=[chunk]
        ) as mock_rerank,
